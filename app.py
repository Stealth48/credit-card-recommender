...
def recommend():
    ...
    if not all([credit_score, current_card, goal, annual_fee, rewards_pref]):
        return render_template('index.html', error="Please fill out all fields to get recommendations.")
    ...
    - Why it’s recommended: Explain how it fits the user’s profile.
    ...
    - Why it’s preferred: Explain why this one stands out for the user.
    ...
    try:
        ...
        response.raise_for_status()  # Raise exception for bad status codes (e.g., 400, 500)
...
if __name__ == '__main__':
    import os
    port = int(os.environ.get("PORT", 5000))  # Render sets PORT, defaults to 5000 locally
